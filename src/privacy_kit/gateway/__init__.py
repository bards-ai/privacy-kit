...
rehydrates the response, and records an audit row (metadata plus the request
text segments selected by ``PII_SAVE_TEXTS``). An OTLP sink scrubs telemetry
the same way (one-way, metadata-only audit).
