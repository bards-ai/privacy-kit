...
after `make route` so its traffic flows through the gateway. (Cursor is routed
from its own Settings UI — run `privacy-kit setup cursor` for instructions.)
Undo routing with `make route-remove`.
