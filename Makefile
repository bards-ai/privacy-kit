...
.PHONY: help install install-all check lint type test test-model fmt \
	...
	route route-remove route-claude-code route-codex route-claude-code-remove route-codex-remove \
...
route: route-claude-code route-codex ## Route Claude Code + Codex through the gateway
...
route-remove: route-claude-code-remove route-codex-remove ## Undo: stop routing tools through the gateway
